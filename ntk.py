...
def ntk_train(X, y, reg=0.1, chunk_size=20, scale=1.0):
    G = ntk(X, X, chunk_size=chunk_size)
    Gi = torch.inverse(G + torch.eye(X.size(0)) * reg)
    lbl = (y==1) * 1.0
    alpha = Gi @ lbl
    return alpha

def ntk_pred(X_train, X_test, alpha, chunk_size=20):
    K = ntk(X_train, X_test, chunk_size=chunk_size)
    pred = alpha @ K
    return pred
