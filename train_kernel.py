...
if __name__ == "__main__":
    ...
    # Training model
    parser.add_argument('--kernel', type=str, default='ntk')
    ...
    # Model
    from kernel import kernel_train, kernel_pred

    alpha = kernel_train(train_data, train_label, k=args.kernel, chunk_size=40, reg=args.reg)
    ...
    for batch_idx in range(args.test_num):
        ...
        test_label = lbl
        pred = kernel_pred(train_data, test_data, alpha, k=args.kernel)
    ...
    #
    with open(f'{args.kernel}_log_{args.man_dim}.txt', 'a') as ff:
